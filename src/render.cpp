...
std::vector<Ray> generatePixelRaysMultisampled(RenderState& state, const Trackball& camera, glm::ivec2 pixel, glm::ivec2 screenResolution)
{
    ...
    for (int sample = 0; sample < numSamples; sample++) {
        ...
        //I retrieved all information I needed to do this from the pre-implemented generatePixelRays function.
...
std::vector<Ray> generatePixelRaysStratified(RenderState& state, const Trackball& camera, glm::ivec2 pixel, glm::ivec2 screenResolution)
{
    ...
    for (uint32_t y = 0; y < numSamples; y++) {
        for (uint32_t x = 0; x < numSamples; x++) {
            ...
            //In other words the first part finds the top left corner of the cell and the second part finds the location within that cell
            ...
            //Convert to NDC: Map to 0,1, then 0,2, then -1,1
